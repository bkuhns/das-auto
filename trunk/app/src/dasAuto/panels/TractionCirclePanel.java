...
public class TractionCirclePanel extends DataPanel {
	...
	public void paint(Graphics g) {
		...
		tracCircleG.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
	...
	private void drawMaxCircle(Graphics2D g) {
		...
		g.draw(new Ellipse2D.Double(origin.x - scaleX(Math.abs(mirroredYMax)),
				origin.y - scaleY(Math.abs(mirroredXMax)), scaleX(2.0 * Math
						.abs(mirroredYMax)), scaleY(2.0 * Math
						.abs(mirroredXMax))));
	...
	private void drawAccelPoints(Graphics2D g) {
		...
		// Draw the points in
		currentSample = accelFeed.getFilteredFeed(25).getNearestSample(
				currentTimestamp);
		/*
		 * long backSampleTime = accelFeed.getFilteredFeed(25).getNearestSample(currentTimestamp).getTimestamp() - 100; int backSampleIter = 1; int alphaValue = 150;
		 * int numTrailDots = 10;
		 * 
		 * while(backSampleTime > 0.0 && backSampleIter <= numTrailDots) {
		 * 		previousSample = accelFeed.getFilteredFeed(25).getNearestSample(backSampleTime);
		 * 		alphaValue = alphaValue - (int)(150.0 / numTrailDots); g.setPaint(new Color(0,0,255,alphaValue));
		 * 
		 * 		this.drawPoint(origin.x + scaleX(-1 * previousSample.getYValueInG()), origin.y + scaleY(previousSample.getXValueInG()), g);
		 * 
		 * 		backSampleTime -= 100; ++backSampleIter;
		 * }
		 */
